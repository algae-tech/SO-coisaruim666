...
#if SCHED_ALGO == SCHED_ORIGINAL || SCHED_ALGO == SCHED_FCFS || SCHED_ALGO == SCHED_RR
...
#elif SCHED_ALGO == SCHED_PRIORITY
  {
    struct proc *best = NIL_PROC;
    int best_prio = 999;
    for (rp = rdy_head[USER_Q]; rp != NIL_PROC; rp = rp->p_nextready) {
	if (rp->p_upriority < best_prio) {
	    best_prio = rp->p_upriority;
	    best = rp;
	}
    }
    if (best != NIL_PROC) {
...
#elif SCHED_ALGO == SCHED_LOTTERY
  {
    struct proc *rp2;
    unsigned long total = 0, winner, acc;
    static unsigned long seed = 12345;

    for (rp2 = rdy_head[USER_Q]; rp2 != NIL_PROC; rp2 = rp2->p_nextready)
	total += rp2->p_tickets;

    if (total > 0) {
	seed = seed * 1103515245 + 12345;
	winner = (seed >> 16) % total;
	acc = 0;
	for (rp2 = rdy_head[USER_Q]; rp2 != NIL_PROC; rp2 = rp2->p_nextready) {
	    acc += rp2->p_tickets;
	    if (acc > winner) {
...
#elif SCHED_ALGO == SCHED_SJF
  {
    struct proc *best = NIL_PROC;
    long best_est = 0x7FFFFFFF;
    for (rp = rdy_head[USER_Q]; rp != NIL_PROC; rp = rp->p_nextready) {
	if (rp->p_burst_estimate < best_est) {
	    best_est = rp->p_burst_estimate;
	    best = rp;
	}
    }
    if (best != NIL_PROC) {
...
#elif SCHED_ALGO == SCHED_FCFS || SCHED_ALGO == SCHED_RR
  /* FCFS e RR: usuario entra no FIM da fila (ordem de chegada). A
   * diferenca entre FCFS e RR esta no clock.c (RR mantem a preempcao
   * por quantum ligada, FCFS desliga). */
  if (rdy_head[USER_Q] != NIL_PROC)
...
#elif SCHED_ALGO == SCHED_PRIORITY
  /* Priority: atribui prioridade estatica com base no numero do
   * processo (na falta de uma interface pro usuario escolher), e
   * insere na fila -- pick_proc() varre procurando o de maior
   * prioridade (menor numero). */
  if (rp->p_upriority == 0) rp->p_upriority = (rp->p_nr % 5) + 1;
  if (rdy_head[USER_Q] == NIL_PROC) {
...
#elif SCHED_ALGO == SCHED_LOTTERY
  /* Lottery: atribui bilhetes com base no numero do processo. */
  if (rp->p_tickets == 0) rp->p_tickets = ((rp->p_nr % 5) + 1) * 10;
  if (rdy_head[USER_Q] == NIL_PROC) {
	rdy_head[USER_Q] = rdy_tail[USER_Q] = rp;
  } else {
	rdy_tail[USER_Q]->p_nextready = rp;
...
#elif SCHED_ALGO == SCHED_SJF
  /* SJF: fila em qualquer ordem, pick_proc() escolhe quem tem a
   * menor estimativa de rajada. */
  if (rdy_head[USER_Q] == NIL_PROC) {
	rdy_head[USER_Q] = rdy_tail[USER_Q] = rp;
  } else {
	rdy_tail[USER_Q]->p_nextready = rp;
...
#if SCHED_ALGO == SCHED_PRIORITY || SCHED_ALGO == SCHED_LOTTERY || SCHED_ALGO == SCHED_SJF
  /* nesses algoritmos a ordem da fila nao importa -- pick_proc() ja
   * varre tudo e escolhe o melhor de novo. */
  {
    long update_actual = get_uptime() - proc_ptr->p_burst_start;
    proc_ptr->p_burst_estimate = (update_actual + proc_ptr->p_burst_estimate) / 2;
    proc_ptr->p_burst_start = get_uptime();
  }
  pick_proc();
#else
  /* ORIGINAL e RR: gira a fila (round-robin classico) */
